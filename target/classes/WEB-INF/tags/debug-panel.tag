...
<jstl:if test="${showPanel}">
	<%
		...
		while (keys.hasMoreElements()) {
			...
			else if (JspHelper.isSpecialName(name)) 
				JspHelper.appendDebugPanelData(otherData, name, value);
			else {
				if (StringHelper.anyOf(name, "$request|$response"))
			...
			<jstl:if test="${!workAreaData.isEmpty()}">
				...
				<jstl:forEach var="workarea" items="${workAreaData.entrySet()}">
					<div class="alert alert-info">
						...
						<jstl:forEach var="tuple" items="${workarea.value[0]}">
							<div><pre><jstl:out value="${tuple}"/></pre></div>							
						</jstl:forEach>					
					</div>
				</jstl:forEach>
			</jstl:if>
