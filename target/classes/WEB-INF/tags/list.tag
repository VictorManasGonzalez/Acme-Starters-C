...
<%
	...
	request.setAttribute("$list$columns", dataTableColumns);
%>
...
<table id="list" class="table table-striped table-condensed table-hover display nowrap w-100">
	...
	<thead>
		<tr>			
			...
			<jstl:forEach var="column" items="${$list$columns}">				
				<th style="width: ${column.width}; text-align: left" data-format="${column.format}">
					...
					<jstl:if test="${column.code == null}">
						<acme:print code="${column.path}"/>
					</jstl:if>
					<jstl:if test="${fetch == null }">
						<jstl:set var="dataEntry" value=""/>
					</jstl:if>
					<jstl:if test="${fetch != null }">
						<jstl:set var="dataEntry" value="'data': '${column.path}', "/>
					</jstl:if>
					<jstl:set var="aoColumns" value="${aoColumns}${comma} { ${dataEntry} 'bSortable': ${column.sortable}, 'bSearchable': true, 'bVisible': ${column.visible}, 'render': $.fn.dataTable.render.text() }"/>
					<jstl:set var="comma" value=","/>
				</th>
			</jstl:forEach>
	...
	<tbody>
		<jstl:if test="${fetch == null && $list$number$data != null && $list$number$data >= 1}">
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		table = $("#list").
				...
				<jstl:if test="${$list$columns.size() > 0}">
				...
				<jstl:if test="${fetch != null}">
				    ...
				    "serverSide": true,
				  	    ...
				  	    "contentType": "application/json; charset=utf-8",
				  	         "render": function(data, type, row) { return ('<div>').text(data).html(); } 
				  	    /*
		...
		<jstl:if test="${navigable}">
		    $("#list tbody").on("click", "li", dataTablesClickListener);
		    $("#list tbody").on("click", "td", dataTablesClickListener);
		</jstl:if>
	});	
</script>
