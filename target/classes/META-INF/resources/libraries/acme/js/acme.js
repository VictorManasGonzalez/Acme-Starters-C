...
function grabIdAsQuery(form) {
	var result;
	var element, value;
	
	element = form.querySelector('#id');
	value = element === null ? null : element.value;
	if (value === null || value === "0")
		result = "";
	else
		result = "id={0}".format(value);
 
	return result;
}

function dataTablesClickListener(event) {
	var id;
	var styles, content, element;
	...
	if (content && content === "none") {
		...
		assert($(this).is("td") || $(this).is("li"), "Wrong click target!")
		if ($(this).is("td.child") || $(this).is("li"))
...
function glueUrlExtraQuery(url, extraQuery) {
	var result;
	var hashIndex, base, frag, queryIndex;
	var glue;


	if (extraQuery == null || String(extraQuery).trim() === "" || /[?&]id=/.test(url))
		result = url;
	else {
		hashIndex = url.indexOf("#");
		base = hashIndex >= 0 ? url.slice(0, hashIndex) : url;
		frag = hashIndex >= 0 ? url.slice(hashIndex) : "";
		queryIndex = base.indexOf('?');
		glue = '';

		if (queryIndex === -1)
			glue = /^[a-z][a-z0-9+.-]*:\/\/[^/?#]+$/i.test(base) ? "/?" : "?";
		else
			glue = (base.endsWith("?") || base.endsWith("&")) ? "" : "&";

		result = base + glue + extraQuery + frag;
	}
...
function getAbsoluteUrl(url, extraQuery) {
	var result;
	...
	result = glueUrlExtraQuery(result, extraQuery);
...
function injectMetadata(name, content) {
	var meta;
...
function injectSignature() {
	...
	input = document.querySelector("form input[type='hidden'][name='_signature']");
	if (input !== null) {
		injectMetadata("signature", input.value);
	}
}